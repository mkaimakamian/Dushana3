...
Public Class LogDAL
    ...
    Public Function GetLog(ByRef since As String, ByRef until As String, level As String, ByRef entity As String) As List(Of LogDTO)
        Dim dbsql As New DBSql
        Dim sql As String
        Dim reader As List(Of List(Of String))
        Dim result As New List(Of LogDTO)

        sql = "SELECT * FROM log WHERE level = '" + level + "' AND created BETWEEN '" + since + "' AND '" + until + "' "
